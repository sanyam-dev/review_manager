...
Backend: FastAPI with clean REST endpoints + OpenAPI docs at /docs. FastAPI provides automatic, interactive docs (Swagger UI). FastAPI


AI: use an LLM API if you have a key; otherwise use a tiny Hugging Face transformers pipeline (e.g., sentiment-analysis, summarization). Hugging Face+1


Search/RAG-lite: implement TF-IDF + cosine similarity to retrieve similar reviews for a query. Use scikit-learn’s TfidfVectorizer and cosine_similarity. Scikit-learn+1
...
5) ## Similar Reviews (RAG-lite)
	Endpoint GET /search?q= returns top-5 similar reviews using TF-IDF vectors and cosine similarity. Scikit-learn+1
...
Include automatic docs at /docs (Swagger UI) and /redoc. FastAPI generates these by default. FastAPI
...
If using Streamlit, you may still keep FastAPI as a separate backend and call it from Streamlit (recommended to exercise both FE and BE). Deployment docs for Streamlit Community Cloud are here. docs.streamlit.io

AI Integration (pick one or support both)
LLM API path (e.g., OpenAI/Claude) — read keys from env; expose basic knobs (temperature/system prompt/few-shots).


Local fallback — use transformers pipelines for:
...
summarization → a short gist to seed the reply
 (Any small model is fine; keep downloads light.) Hugging Face+1



Deployment (required)
Provide a public URL. Choose one:
React frontend on Vercel (quickstart here) or Netlify. Vercel+1


Streamlit app on Streamlit Community Cloud (if you built Streamlit). docs.streamlit.io


If using React + FastAPI, deploy the frontend on Vercel or Netlify, and deploy the backend anywhere you like (Render/Railway/Fly/etc.). Vercel/Netlify docs cover the React deploy steps; follow their CLI or Git-based flow. Vercel+1
(If you prefer deploying React on Vercel via CLI, see vercel --prod guide. Vercel)
...
Search/RAG-lite (10) — TF-IDF + cosine similarity implemented correctly. Scikit-learn+1
...
Submission
Folder Zip file with:


README.md (setup, .env sample, how to run tests, how you deployed, time spent + trade-offs).


Minimal seed dataset at data/reviews.json (50–200 rows).


requests.http or curl examples for each API route.


Live URL(s):


Frontend (Vercel/Netlify/Streamlit) — required. Vercel+2Netlify Docs+2


Backend (if separate) — optional to expose; if private, share a short screen-capture demo.


Short screen-capture (≤3 min) walking through flows and design choices.
...
FastAPI generates interactive docs at /docs (Swagger UI) by default; don’t over-engineer. FastAPI


For TF-IDF search, TfidfVectorizer + cosine_similarity is sufficient. Scikit-learn+1


For a quick AI fallback, use transformers pipelines (pipeline('sentiment-analysis'), pipeline('summarization')). Hugging Face+1
